...
let currentWeek = '3-4';
let mealData = {};
let isRebalancing = false; // Prevent infinite loops
...
// REBALANCE FUNCTION - When user manually edits a quantity
function rebalance(inputElement) {
    if (isRebalancing) return; // Prevent recursive calls
    isRebalancing = true;
    
    // Get meal info from input ID (e.g., "b1-qty" → meal="b", slot=1)
    const id = inputElement.id;
    const mealPrefix = id.charAt(0) + (id.charAt(1) === 'w' || id.charAt(1) === 'o' ? id.charAt(1) : '');
    const mealName = mealPrefix === 'b' ? 'breakfast' :
                     mealPrefix === 'l' ? 'lunch' :
                     mealPrefix === 'pw' ? 'preworkout' :
                     mealPrefix === 'po' ? 'postworkout' : 'dinner';
    
    // Recalculate this meal with user's edited values
    recalculateMealWithEdits(mealName);
    
    isRebalancing = false;
}

// Recalculate meal considering user edits
function recalculateMealWithEdits(mealName) {
    const t = TARGETS[currentWeek][mealName];
    if (!t) return;
    
    const prefix = mealName === 'breakfast' ? 'b' :
                   mealName === 'lunch' ? 'l' :
                   mealName === 'preworkout' ? 'pw' :
                   mealName === 'postworkout' ? 'po' : 'd';
    
    const slots = mealName === 'breakfast' ? [1,2,3,4] :
                  mealName === 'lunch' ? [1,2,3,4,5] :
                  mealName === 'preworkout' ? [1] :
                  mealName === 'postworkout' ? [1,2] : [1,2,3,4];
    
    // Get all selected items with their current quantities
    const items = [];
    let lockedQty = 0, lockedP = 0, lockedC = 0, lockedF = 0, lockedCal = 0;
    
    slots.forEach(s => {
        const select = document.getElementById(prefix + s);
        const qtyInput = document.getElementById(prefix + s + '-qty');
        
        if (select && select.value && qtyInput) {
            const food = FOOD_DATABASE.find(f => f.name === select.value);
            const qty = parseInt(qtyInput.value) || 0;
            
            if (food) {
                const m = qty / 100;
                lockedP += food.protein * m;
                lockedC += food.carbs * m;
                lockedF += food.fats * m;
                lockedCal += food.calories * m;
    ...
    if (items.length === 0) {
        clearMeal(prefix, slots.length);
        return;
    }
    
    // Calculate gaps
    const proteinGap = (t.protein || 0) - lockedP;
    const carbGap = (t.carbs || 0) - lockedC;
    
    // If gaps are small, just update displays
    if (Math.abs(proteinGap) < 2 && Math.abs(carbGap) < 2) {
        items.forEach(item => {
            displayItem(prefix + item.slot, item.qty, item.food);
        });
        updateMealTotal(prefix, items);
        updateDailySummary();
        return;
    }
    
    // Find best item to adjust (highest density for the nutrient we need)
    let bestItem = null;
    if (Math.abs(carbGap) > Math.abs(proteinGap)) {
        // Need to adjust carbs more
        items.forEach(item => {
            if (!bestItem || item.food.carbs > bestItem.food.carbs) {
                bestItem = item;
            }
        });
        
        if (bestItem) {
            const adjustment = (carbGap / bestItem.food.carbs) * 100;
            const newQty = Math.max(10, Math.round(bestItem.qty + adjustment));
            bestItem.qtyInput.value = newQty;
            bestItem.qty = newQty;
        }
    } else {
        // Need to adjust protein more
        items.forEach(item => {
            if (!bestItem || item.food.protein > bestItem.food.protein) {
                bestItem = item;
            }
        });
        
        if (bestItem) {
            const adjustment = (proteinGap / bestItem.food.protein) * 100;
            const newQty = Math.max(10, Math.round(bestItem.qty + adjustment));
            bestItem.qtyInput.value = newQty;
            bestItem.qty = newQty;
        }
...
function updateMealTotal(prefix, items) {
    let totalP = 0, totalC = 0, totalF = 0, totalCal = 0;
    
    items.forEach(item => {
        const m = item.qty / 100;
        totalP += item.food.protein * m;
        totalC += item.food.carbs * m;
        totalF += item.food.fats * m;
        totalCal += item.food.calories * m;
    });
    
    const mealName = prefix === 'b' ? 'breakfast' :
                     prefix === 'l' ? 'lunch' :
                     prefix === 'pw' ? 'preworkout' :
                     prefix === 'po' ? 'postworkout' : 'dinner';
    
    displayTotal(prefix, totalP, totalC, totalF, totalCal);
    
    const t = TARGETS[currentWeek][mealName];
    if (t) {
        displayStatus(prefix, totalP, totalC, t.protein, t.carbs);
    }
    
    mealData[mealName] = {totalP, totalC, totalF, totalCal};
}
...
function displayItem(id, qty, food) {
    ...
    if (qtyInput) qtyInput.value = Math.round(qty);
    if (macrosEl) macrosEl.textContent = `P:${p} C:${c} F:${f}`;
}
